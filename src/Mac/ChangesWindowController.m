...
- (IBAction)upload:(id)sender {
    ...
    if (appDelegate.userName.length == 0 || appDelegate.userPassword.length == 0) {
        NSAlert *alert = [NSAlert alertWithMessageText:@"Before uploading data you must enter a username and password in the Preferences window"
                                         defaultButton:@"OK"
                                       alternateButton:nil
                                           otherButton:nil
    ...
    [_mapData uploadChangesetWithComment:comment
                                 imagery:nil
