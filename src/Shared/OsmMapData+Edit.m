...
- (EditActionReturnNode)canMergeNode:(OsmNode *)node1 intoNode:(OsmNode *)node2 error:(NSString **)error {
    ...
    for (OsmRelation *relation in restrictions) {
        ...
        for (OsmNode *n in nodes) {
            if ([from containsObject:n]) {
                connectFrom = true;
            }
            if ([via containsObject:n]) {
                connectVia = true;
            }
            if ([to containsObject:n]) {
                connectTo = true;
            }
            if ([keyfrom containsObject:n]) {
                connectKeyFrom = true;
            }
            if ([keyto containsObject:n]) {
                connectKeyTo = true;
            }
        }
        ...
        if (connectKeyFrom || connectKeyTo) {
            ...
            for (OsmWay *way in memberWays) {
                if ([way.nodes containsObject:nodes[0]]) {
                    n0 = nodes[0];
                }
                if ([way.nodes containsObject:nodes[1]]) {
                    n1 = nodes[1];
                }
            }
