...
- (instancetype)initWithMapView:(MapView *)mapView {
    ...
    if (self) {
        ...
        [_mapView addObserver:self
                   forKeyPath:@"screenFromMapTransform"
                      options:0
                      context:NULL];
    }
    return self;
...
- (BOOL)fetchTileForTileX:(int32_t)tileX tileY:(int32_t)tileY
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^(void) {
      ...
      if (fileImage) {
          ...
          [[DownloadThreadPool generalPool] dataForUrl:url
                                        completeOnMain:NO
                                            completion:^(NSData *data, NSError *error) {
                                              ...
                                              if (error) {
                                                  ...
                                                  error = [NSError errorWithDomain:@"Image"
                                                                              code:100
                                                                          userInfo:@{NSLocalizedDescriptionKey : NSLocalizedString(@"No image data at current zoom level", nil),
                                                                                     @"Ignorable" : @(YES)}];
                                              } else {
                                  ...
                                  [self removeUnneededTilesForRect:rc zoomLevel:zoomLevel];
                                                  ...
                                                  dispatch_async(dispatch_get_main_queue(), ^(void) {
                                                    [self fetchTileForTileX:tileX >> 1
                                                                      tileY:tileY >> 1
                                                                    minZoom:minZoom
