...
- (void)viewDidAppear {
    ...
    [displayLink addName:@"autoScroll"
                   block:^{
                     int dx = 0, dy = 0;
                     switch (side) {
                     case 0:
...
- (void)presentError:(NSError *)error flash:(BOOL)flash {
    if (_lastErrorDate == nil || [[NSDate date] timeIntervalSinceDate:_lastErrorDate] > 3.0) {
        ...
        if ([error.domain isEqualToString:@"HTTP"] && error.code >= 400 && error.code < 500 && error.localizedDescription.length > 0 && [error.localizedDescription hasPrefix:@"<"]) {
        ...
        if (flash) {
            ...
            [alertError addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", nil) style:UIAlertActionStyleCancel handler:nil]];
            if (ignoreButton) {
                [alertError addAction:[UIAlertAction actionWithTitle:ignoreButton
...
- (void)askToRate:(NSInteger)uploadCount {
    ...
    if (uploadCount > 1 && countLog10 == floor(countLog10)) {
        ...
        [alertViewRateApp addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Maybe later...", nil)
                                                             style:UIAlertActionStyleCancel
                                                           handler:^(UIAlertAction *action){
                                                           }]];
        [alertViewRateApp addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"I'll do it!", nil)
                                                             style:UIAlertActionStyleDefault
                                                           handler:^(UIAlertAction *action) {
                                                             [self showInAppStore];
                                                           }]];
...
- (void)locationManager:(CLLocationManager *)manager didUpdateHeading:(CLHeading *)newHeading {
    ...
    [[DisplayLink shared] addName:@"smoothHeading"
                            block:^{
...
- (void)animateRotationBy:(double)deltaHeading aroundPoint:(CGPoint)center {
    ...
    [displayLink addName:DisplayLinkHeading
                   block:^{
...
- (IBAction)paste:(id)sender {
    ...
    if (_editorLayer.selectedPrimary.tags.count > 0) {
        ...
        [alertPaste addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Merge Tags", nil)
                                                       style:UIAlertActionStyleDefault
                                                     handler:^(UIAlertAction *alertAction) {
                                                       [_editorLayer mergeTags:_editorLayer.selectedPrimary];
                                                       [self refreshPushpinText];
                                                     }]];
        [alertPaste addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Replace Tags", nil)
                                                       style:UIAlertActionStyleDefault
                                                     handler:^(UIAlertAction *alertAction) {
...
- (IBAction)delete:(id)sender {
    ...
    if (_editorLayer.selectedRelation.isMultipolygon && _editorLayer.selectedPrimary.isWay) {
        ...
        [alertDelete addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", nil)
                                                        style:UIAlertActionStyleCancel
                                                      handler:^(UIAlertAction *action){
                                                      }]];
        ...
        [alertDelete addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Detach from relation", nil)
                                                        style:UIAlertActionStyleDefault
                                                      handler:^(UIAlertAction *action) {
                                                        NSString *error = nil;
...
NSString *ActionTitle(EDIT_ACTION action, BOOL abbrev) {
    ...
    case ACTION_RECTANGULARIZE:
        return NSLocalizedString(@"Make Rectangular", nil);
    case ACTION_STRAIGHTEN:
        return NSLocalizedString(@"Straighten", nil);
    case ACTION_REVERSE:
        return NSLocalizedString(@"Reverse", nil);
    case ACTION_DUPLICATE:
        return NSLocalizedString(@"Duplicate", nil);
    case ACTION_ROTATE:
        return NSLocalizedString(@"Rotate", nil);
    case ACTION_CIRCULARIZE:
        return NSLocalizedString(@"Make Circular", nil);
    case ACTION_JOIN:
        return NSLocalizedString(@"Join", nil);
    case ACTION_DISCONNECT:
        return NSLocalizedString(@"Disconnect", nil);
    case ACTION_COPYTAGS:
        return NSLocalizedString(@"Copy Tags", nil);
    case ACTION_PASTETAGS:
        return NSLocalizedString(@"Paste", nil);
    case ACTION_EDITTAGS:
        return NSLocalizedString(@"Tags", nil);
    case ACTION_ADDNOTE:
        return NSLocalizedString(@"Add Note", nil);
    case ACTION_DELETE:
        return NSLocalizedString(@"Delete", nil);
    case ACTION_MORE:
        return NSLocalizedString(@"More...", nil);
    case ACTION_HEIGHT:
        return NSLocalizedString(@"Measure Height", nil);
    case ACTION_RESTRICT:
        return abbrev ? NSLocalizedString(@"Restrict", nil) : NSLocalizedString(@"Turn Restrictions", nil);
    case ACTION_CREATE_RELATION:
        return NSLocalizedString(@"Create Relation", nil);
    };
    return nil;
}
...
- (void)performEditAction:(EDIT_ACTION)action {
    ...
    case ACTION_CREATE_RELATION: {
        ...
        [actionSheet addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Multipolygon", nil)
                                                        style:UIAlertActionStyleDefault
    ...
    void (^restrictionEditWarning)(OsmNode *) = ^(OsmNode *viaNode) {
      ...
      if (warn) {
          UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Unsupported turn restriction type"
                                                                  ...
                                                                  preferredStyle:UIAlertControllerStyleAlert];
          [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Edit restrictions", nil)
        ...
        _pushpinView.dragCallback = ^(UIGestureRecognizerState state, CGFloat dx, CGFloat dy, UIGestureRecognizer *gesture) {
          ...
          case UIGestureRecognizerStateEnded: {
              ...
              if (strongSelf->_confirmDrag) {
                  ...
                  [alertMove addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Undo", nil)
                                                                style:UIAlertActionStyleCancel
                                                              handler:^(UIAlertAction *action) {
                                                                // cancel move
                  ...
                  [alertMove addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Move", nil)
                                                                style:UIAlertActionStyleDefault
                                                              handler:^(UIAlertAction *action){
                                                                  // okay
                                                              }]];
          ...
          case UIGestureRecognizerStateChanged: {
              ...
              if (scrollx || scrolly) {
                  ...
                  [displayLink addName:@"dragScroll"
                                 block:^{
...
- (void)updateNotesFromServerWithDelay:(CGFloat)delay {
    if (_viewOverlayMask & VIEW_OVERLAY_NOTES) {
        ...
        [_notesDatabase updateRegion:rc
                           withDelay:delay
                           fixmeData:self.editorLayer.mapData
                          completion:^{
                            [self refreshNoteButtonsFromDatabase];
                          }];
    } else {
...
- (void)noteButtonPress:(id)sender {
    ...
    if (note.isWaypoint || note.isKeepRight) {
        ...
        [alertKeepRight addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", nil)
                                                           style:UIAlertActionStyleCancel
                                                         handler:^(UIAlertAction *action){
                                                         }]];
        [alertKeepRight addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Ignore", nil)
                                                           style:UIAlertActionStyleDefault
                                                         handler:^(UIAlertAction *action) {
                                                           // they want to hide this button from now on
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)pan {
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (hypot(initialVelecity.x, initialVelecity.y) < 100.0) {
            ...
            __weak MapView *weakSelf = self;
            __weak DisplayLink *displayLink = [DisplayLink shared];
            [displayLink addName:DisplayLinkPanning
                           block:^{
                             MapView *myself = weakSelf;
                             if (myself) {
    ...
    if (longPress.state == UIGestureRecognizerStateBegan && !_editorLayer.hidden) {
        ...
        if (_editorLayer.selectedPrimary.isRelation.isMultipolygon) {
            ...
            if (ways.count == 1) {
                ...
                [confirm addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Add outer member", nil)
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *_Nonnull action) {
                                                            addMmember(@"outer");
                                                          }]];
                [confirm addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Add inner member", nil)
                                                            style:UIAlertActionStyleDefault
                                                          handler:^(UIAlertAction *_Nonnull action) {
                                                            addMmember(@"inner");
                                                          }]];
        ...
        for (OsmBaseObject *object in objects) {
            ...
            [multiSelectSheet addAction:[UIAlertAction actionWithTitle:title
                                                                 style:UIAlertActionStyleDefault
                                                               handler:^(UIAlertAction *_Nonnull action) {
