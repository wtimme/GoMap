...
#if DEBUG
        if (tags.count)
            assert([tags.lastObject isKindOfClass:[CommonTagKey class]] ||
...
- (CommonTagGroup *)groupForField:(NSString *)fieldName geometry:(NSString *)geometry update:(void (^)(void))update {
    ...
    if ([type isEqualToString:@"defaultcheck"] || [type isEqualToString:@"check"] || [type isEqualToString:@"onewayCheck"]) {
        ...
        NSArray *presets = @[ [CommonTagValue presetWithName:[CommonTagList yesForLocale] details:nil tagValue:@"yes"],
                              [CommonTagValue presetWithName:[CommonTagList noForLocale]
                                                     details:nil
                                                    tagValue:@"no"] ];
        ...
        if (stringsOptionsDict) {
            ...
            if (g_taginfoCache[fieldName]) {
                ...
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
                  ...
                  NSString *urlText = isMulti ? [NSString stringWithFormat:@"https://taginfo.openstreetmap.org/api/4/keys/all?query=%@&filter=characters_colon&page=1&rp=10&sortname=count_all&sortorder=desc", cleanKey] : [NSString stringWithFormat:@"https://taginfo.openstreetmap.org/api/4/key/values?key=%@&page=1&rp=25&sortname=count_all&sortorder=desc", key];
