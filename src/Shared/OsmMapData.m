...
- (void)setupPeriodicSaveTimer {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:UndoManagerDidChangeNotification
                                                      object:_undoManager
                                                       queue:nil
...
- (NSArray *)userStatisticsForRegion:(OSMRect)rect {
    ...
    [self enumerateObjectsInRegion:rect
                             block:^(OsmBaseObject *base) {
...
#else
        [self clearCachedProperties:parent
                               undo:_undoManager];
#endif
...
- (void)addChangeCallback:(void (^)(void))callback {
    [[NSNotificationCenter defaultCenter] addObserverForName:UndoManagerDidChangeNotification
                                                      object:_undoManager
                                                       queue:nil
                                                  usingBlock:^(NSNotification *_Nonnull fnote) {
                                                    callback();
                                                  }];
}
...
+ (void)osmDataForUrl:(NSString *)url quads:(ServerQuery *)quads completion:(void (^)(ServerQuery *quads, OsmMapData *data, NSError *error))completion {
    [[DownloadThreadPool osmPool] streamForUrl:url
                                      callback:^(NSInputStream *stream, NSError *error2) {
...
+ (NSXMLElement *)elementForObject:(OsmBaseObject *)object {
    NSString *type = object.isNode ? @"node" : object.isWay ? @"way" : object.isRelation ? @"relation" : nil;
    assert(type);
...
- (void)uploadChangesetXML:(NSXMLDocument *)xmlChanges changesetID:(NSString *)changesetID retries:(NSInteger)retries completion:(void (^)(NSString *errorMessage))completion {
    ...
    [self putRequest:url2
              method:@"POST"
                 xml:xmlChanges
          completion:^(NSData *postData, NSString *postErrorMessage) {
            ...
            if (retries > 0 && [response hasPrefix:@"Version mismatch"]) {
                ...
                if (sscanf(response.UTF8String, "Version mismatch: Provided %d, server had: %d of %[a-zA-Z] %lld", &localVersion, &serverVersion, type, &objId) == 4) {
                    ...
                    [OsmMapData osmDataForUrl:url3
                                        quads:nil
                                   completion:^(ServerQuery *quads, OsmMapData *mapData, NSError *error) {
            ...
            [self putRequest:url3
                      method:@"PUT"
                         xml:nil
                  completion:^(NSData *data, NSString *errorMessage) {
                    if (errorMessage)
...
- (void)createChangesetWithComment:(NSString *)comment imagery:(NSString *)imagery completion:(void (^)(NSString *changesetID, NSString *errorMessage))completion {
    ...
    [self putRequest:url
              method:@"PUT"
                 xml:xmlCreate
          completion:^(NSData *putData, NSString *putErrorMessage) {
...
- (void)uploadChangesetWithComment:(NSString *)comment imagery:(NSString *)imagery completion:(void (^)(NSString *errorMessage))completion {
    [self createChangesetWithComment:comment
                             imagery:imagery
                          completion:^(NSString *changesetID, NSString *errorMessage) {
                            if (changesetID) {
