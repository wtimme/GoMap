...
- (void)updateMapLocation {
    ...
    [_mapData updateWithBox:box
                    mapView:_mapView
                 completion:^(BOOL partial, NSError *error) {
                   if (error) {
                       dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)invokeAlongScreenClippedWay:(OsmWay *)way offset:(double)initialOffset interval:(double)interval block:(void (^)(OSMPoint pt, OSMPoint direction))block {
    ...
    [self invokeAlongScreenClippedWay:way
                                block:^BOOL(OSMPoint p1, OSMPoint p2, BOOL isEntry, BOOL isExit) {
                                  if (isEntry)
...
- (CGPathRef)pathClippedToViewRect:(OsmWay *)way length:(double *)pLength CF_RETURNS_RETAINED {
    ...
    [self invokeAlongScreenClippedWay:way
                                block:^(OSMPoint p1, OSMPoint p2, BOOL isEntry, BOOL isExit) {
                                  if (path == NULL) {
...
- (NSMutableArray *)getShapeLayersForHighlights {
    ...
    for (OsmBaseObject *object in _shownObjects) {
        if (object.isOneWay || [highlights containsObject:object]) {
            ...
            // arrow heads
            [self invokeAlongScreenClippedWay:object.isWay
                                       offset:50
                                     interval:100
                                        block:^(OSMPoint loc, OSMPoint dir) {
...
- (NSMutableArray *)getVisibleObjects {
    ...
    [_mapData enumerateObjectsInRegion:box
                                 block:^(OsmBaseObject *obj) {
...
- (OsmBaseObject *)osmHitTest:(CGPoint)point radius:(CGFloat)radius testNodes:(BOOL)testNodes ignoreList:(NSArray *)ignoreList segment:(NSInteger *)pSegment {
    ...
    [EditorMapLayer osmHitTestEnumerate:point
                                 radius:radius
                                mapView:_mapView
                                objects:_shownObjects
                              testNodes:testNodes
                             ignoreList:ignoreList
                                  block:^(OsmBaseObject *obj, CGFloat dist, NSInteger segment) {
                                    if (dist < bestDist) {
                                        bestDist = dist;
