...
+ (void)getHasPrefix:(BOOL *)hasPrefixPtr localName:(NSString **)localNamePtr forName:(NSString *)name {
    ...
    if (name) {
        ...
        if (range.length != 0) {
            if (hasPrefixPtr)
                *hasPrefixPtr = range.location > 0;
            if (localNamePtr)
                *localNamePtr = [name substringFromIndex:(range.location + range.length)];
        } else {
            if (hasPrefixPtr)
                *hasPrefixPtr = NO;
            if (localNamePtr)
...
+ (void)getPrefix:(NSString **)prefixPtr localName:(NSString **)localNamePtr forName:(NSString *)name {
    ...
    if (name) {
        ...
        if (range.length != 0) {
            if (prefixPtr)
                *prefixPtr = [name substringToIndex:range.location];
            if (localNamePtr)
                *localNamePtr = [name substringFromIndex:(range.location + range.length)];
        } else {
            if (prefixPtr)
